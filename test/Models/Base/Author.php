...
/**
 * Base class that represents a row from the 'nft__authors' table.
 *
 *
 *
...
abstract class Author implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\Test\\Models\\Map\\AuthorTableMap';
    ...
    /**
     * Initializes internal state of Test\Models\Base\Author object.
     * @see applyDefaults()
     */
    ...
    public function setCrdate($v)
    {
        ...
        if ($this->crdate !== $v) {
            $this->crdate = $v;
            $this->modifiedColumns[AuthorTableMap::COL_CRDATE] = true;
        }
    ...
    public function setTstamp($v)
    {
        ...
        if ($this->tstamp !== $v) {
            $this->tstamp = $v;
            $this->modifiedColumns[AuthorTableMap::COL_TSTAMP] = true;
        }
    ...
    public function setTitle($v)
    {
        ...
        if ($this->title !== $v) {
            $this->title = $v;
            $this->modifiedColumns[AuthorTableMap::COL_TITLE] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : AuthorTableMap::translateFieldName('Crdate', TableMap::TYPE_PHPNAME, $indexType)];
            $this->crdate = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 2 + $startcol : AuthorTableMap::translateFieldName('Tstamp', TableMap::TYPE_PHPNAME, $indexType)];
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::COL_CRDATE)) {
            $modifiedColumns[':p' . $index++]  = 'crdate';
        }
        if ($this->isColumnModified(AuthorTableMap::COL_TSTAMP)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\Test\Models\Author
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\Test\Models\Author The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(AuthorTableMap::COL_CRDATE)) {
            $criteria->add(AuthorTableMap::COL_CRDATE, $this->crdate);
        }
        if ($this->isColumnModified(AuthorTableMap::COL_TSTAMP)) {
            $criteria->add(AuthorTableMap::COL_TSTAMP, $this->tstamp);
        }
        if ($this->isColumnModified(AuthorTableMap::COL_TITLE)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \Test\Models\Author (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \Test\Models\Author Clone of current object.
     * @throws PropelException
     */
    ...
    public function initBooks($overrideExisting = true)
    {
        ...
        $this->collBooks->setModel('\Test\Models\Book');
    }
